...
use enlargecanvas::{
    ...
    EnlargeCanvasHistoryParameters, EnlargeCanvasImageError, EnlargeCanvasParametersV1,
    EnlargeCanvasPlan, EnlargeCanvasPlanError, decode_history,
};
use rusttable_image::{
...
#[test]
fn roi_out_matches_native_f32_truncation_and_bounds() {
    let truncating = EnlargeCanvasPlan::new(
        config(33.333_332, 0.0, 0.0, 0.0, CanvasColor::Green),
        dimensions(10, 10),
    )
    .expect("plan");
    assert_eq!(
        truncating
            .modify_roi_out(rusttable_image::Roi::new(7, 6, 3, 4).expect("ROI"))
            .expect("output ROI"),
        rusttable_image::Roi::new(7, 6, 5, 5).expect("minimum output ROI")
    );

    let bounded = EnlargeCanvasPlan::new(
        config(100.0, 100.0, 100.0, 100.0, CanvasColor::Green),
        dimensions(10, 10),
    )
    .expect("plan");
    assert_eq!(bounded.output_dimensions(), dimensions(30, 30));
    assert_eq!(
        bounded
            .modify_roi_out(rusttable_image::Roi::full(
                ImageDimensions::new(10, 10).expect("dimensions")
            ),)
            .expect("three-times output"),
        rusttable_image::Roi::new(0, 0, 30, 30).expect("output ROI")
    );
}

#[test]
fn scaled_roi_input_preserves_native_rounding() {
    let plan = EnlargeCanvasPlan::new(
        config(50.0, 50.0, 25.0, 75.0, CanvasColor::Green),
        dimensions(100, 80),
    )
    .expect("plan");
    let output = rusttable_image::Roi::new(5, 4, 100, 100).expect("output ROI");
    assert_eq!(
        plan.modify_roi_in_scaled(output, dimensions(100, 80), dimensions(130, 110), 0.5)
            .expect("scaled input ROI"),
        rusttable_image::Roi::new(0, 1, 50, 39).expect("input ROI")
    );

    let clipped = EnlargeCanvasPlan::new(
        config(100.0, 0.0, 100.0, 0.0, CanvasColor::Green),
        dimensions(10, 10),
    )
    .expect("plan");
    assert_eq!(
        clipped
            .modify_roi_in_scaled(
                rusttable_image::Roi::new(0, 0, 5, 5).expect("canvas ROI"),
                dimensions(10, 10),
                dimensions(15, 15),
                1.0,
            )
            .expect("one-pixel input ROI"),
        rusttable_image::Roi::new(0, 0, 1, 1).expect("minimum input ROI")
    );
}

#[test]
fn one_sided_right_bottom_and_minimum_canvas_placement_match_native() {
    let right_bottom = EnlargeCanvasPlan::new(
        config(0.0, 100.0, 0.0, 100.0, CanvasColor::Green),
        dimensions(4, 4),
    )
    .expect("plan");
    assert_eq!(right_bottom.output_dimensions(), dimensions(8, 8));
    assert_eq!(right_bottom.source_offset(), (0, 0));
    assert_eq!(right_bottom.geometry().right(), 4);
    assert_eq!(right_bottom.geometry().bottom(), 4);
    let mut points = [0.5, 1.5, 3.0, 2.0];
    right_bottom
        .forward_transform(&mut points)
        .expect("forward");
    assert!(
        points
            .iter()
    ...
    let minimum = EnlargeCanvasPlan::new(
        config(0.0, 0.0, 0.0, 0.0, CanvasColor::Green),
        dimensions(2, 2),
    )
    .expect("minimum plan");
    assert_eq!(minimum.output_dimensions(), dimensions(5, 5));
    assert_eq!(minimum.source_offset(), (1, 1));
    minimum.forward_transform(&mut points).expect("forward");
    assert!(
        points
            .iter()
            .zip([0.5, 1.5, 3.0, 2.0])
...
#[test]
fn aggregate_ratios_and_placement_clamps_match_native() {
    let plan = EnlargeCanvasPlan::new(
        config(25.0, 75.0, 25.0, 75.0, CanvasColor::Green),
        dimensions(4, 4),
    )
    .expect("plan");
    assert_eq!(plan.output_dimensions(), dimensions(8, 8));
    assert_eq!(plan.source_offset(), (1, 1));
    assert_eq!(plan.geometry().right(), 3);
    assert_eq!(plan.geometry().bottom(), 3);
    assert_eq!(
        plan.source_placement(
            rusttable_image::Roi::new(0, 0, 4, 4).expect("input ROI"),
            rusttable_image::Roi::full(ImageDimensions::new(8, 8).expect("output dimensions")),
        )
        .expect("source placement"),
        (1, 1)
    );

    let clamped = EnlargeCanvasPlan::new(
        config(50.0, 50.0, 50.0, 50.0, CanvasColor::Green),
        dimensions(2, 2),
    )
    .expect("plan");
    assert_eq!(
        clamped
            .source_placement(
                rusttable_image::Roi::new(0, 0, 2, 2).expect("input ROI"),
                rusttable_image::Roi::new(0, 0, 2, 2).expect("small output tile"),
            )
            .expect("clamped source placement"),
        (0, 0)
    );
    let mut points = [0.5, 1.5, 3.0, 2.0];
    clamped.forward_transform(&mut points).expect("forward");
    assert!(
        points
            .iter()
            .zip([1.5, 2.5, 4.0, 3.0])
    ...
    clamped.back_transform(&mut points).expect("back");
    assert!(
        points
            .iter()
            .zip([0.5, 1.5, 3.0, 2.0])
...
fn roi_input_is_only_the_source_intersection() {
    ...
    let source_tile = rusttable_image::Roi::new(3, 1, 2, 2).expect("ROI");
    assert_eq!(
    ...
    assert_eq!(
        plan.modify_roi_out(rusttable_image::Roi::new(0, 0, 3, 2).expect("source ROI"))
            .expect("output ROI"),
        rusttable_image::Roi::new(0, 0, 6, 5).expect("output ROI")
    );
}
...
fn scalar_fill_copy_and_mask_execution_are_deterministic() {
    ...
    assert_eq!(first.dimensions(), dimensions(5, 5));
    assert_eq!(first.pixels()[0], CanvasColor::Red.fill().rgb_pixel());
    assert_eq!(first.pixels()[6], pixel(1.0));
    assert_eq!(first.pixels()[7], pixel(2.0));
    assert_eq!(first.pixels()[11], pixel(3.0));
    assert_eq!(first.pixels()[12], pixel(4.0));
    assert!(matches!(
...
#[test]
fn image_contract_accepts_valid_rgba_and_copies_padded_rows() {
    ...
    assert_eq!(output.descriptor().dimensions().width(), 5);
    assert_eq!(output.descriptor().dimensions().height(), 5);
    assert_eq!(output.descriptor().format(), format);
    assert_eq!(
        output.descriptor().color_encoding(),
        ColorEncoding::LinearSrgbD65
    );
...
#[test]
fn image_contract_rejects_non_rgba_and_non_interleaved_layouts() {
    let plan = EnlargeCanvasPlan::new(
        config(0.0, 0.0, 0.0, 0.0, CanvasColor::Black),
        dimensions(1, 1),
    )
    .expect("plan");
    let formats = [
        ...
        .expect("premultiplied RGBA format"),
    ];
    ...
    for format in formats {
        ...
        .expect("descriptor");
        let input = vec![0_u8; descriptor.byte_length()];
        assert!(matches!(
            plan.execute_image(&descriptor, &input),
            Err(EnlargeCanvasImageError::UnsupportedLayout)
        ));
    }
}

#[test]
fn image_contract_rejects_non_linear_srgb_encoding() {
    let plan = EnlargeCanvasPlan::new(
        config(0.0, 0.0, 0.0, 0.0, CanvasColor::Black),
        dimensions(1, 1),
    )
    ...
    assert!(matches!(
        plan.execute_image(&descriptor, &input),
        Err(EnlargeCanvasImageError::UnsupportedEncoding)
    ));
}
...
fn invalid_parameters_are_rejected_before_planning() {
    ...
    assert!(matches!(
        ...
        .expect("identity")
        .modify_roi_in(rusttable_image::Roi::new(6, 0, 0, 1).expect("ROI")),
