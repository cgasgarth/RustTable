...
pub fn compile_colormapping(
    ...
    let bytes = decode_native_payload_chunks(&chunk_refs, COLOR_MAPPING_PARAMETER_BYTES)
        .map_err(|error| invalid(operation, &error))?;
    let parameters =
        ColorMappingParametersV1::from_bytes(&bytes).map_err(|error| invalid(operation, &error))?;
    let config = ColorMappingConfig::new(parameters).map_err(|error| invalid(operation, &error))?;
    Ok(ProcessingOperation {
        operation_id: operation.id(),
        enabled: operation.is_enabled(),
        opacity: compile_opacity(operation)?,
        kind: ProcessingOperationKind::ColorMapping {
