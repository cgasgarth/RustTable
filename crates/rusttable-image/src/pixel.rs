...
/// A bounded reference to an ICC/profile source rather than untyped bytes.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ColorProfileRef(String);

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ColorProfileRefError {
    Empty,
    TooLong,
}

impl ColorProfileRef {
    /// Creates a bounded profile identifier.
    ///
    /// # Errors
    ///
    /// Returns an error for an empty or overlong reference.
    pub fn new(value: impl Into<String>) -> Result<Self, ColorProfileRefError> {
        let value = value.into();
        if value.is_empty() {
            return Err(ColorProfileRefError::Empty);
        }
        if value.len() > 512 {
            return Err(ColorProfileRefError::TooLong);
        }
        Ok(Self(value))
    }
...
impl fmt::Display for ColorProfileRefError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(match self {
            Self::Empty => "color profile reference must not be empty",
            Self::TooLong => "color profile reference exceeds 512 bytes",
        })
    }
}

impl std::error::Error for ColorProfileRefError {}
