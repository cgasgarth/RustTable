...
cheap_labels=(diff fmt metadata source bun macos-artifact-identity repository-policy cache-workflow-policy pr-branch-freshness doctor readme-contract native-removal linux-distribution)
...
cheap_pids[6]=$!
run_check cache-workflow-policy bash -c 'bun run test:cache-workflow-policy && bun run check:cache-workflow-policy' &
cheap_pids[7]=$!
