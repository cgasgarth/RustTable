...
# The GitHub Linux runner has multiple cores; keep a four-job floor because
# ubuntu-latest can report only two schedulable CPUs while the cold Wasmtime
# test lane is compiling. An explicit caller override remains authoritative.
reported_jobs="$(getconf _NPROCESSORS_ONLN 2>/dev/null || printf '1')"
if [[ "$reported_jobs" =~ ^[0-9]+$ ]] && (( reported_jobs < 4 )); then
  reported_jobs=4
fi
