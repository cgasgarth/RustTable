...
success_output="$(FAKE_ROOT="$fixture/repo" FAKE_LOG="$fixture/log" PATH="$fixture/bin:$PATH" bash "$script")"
[[ "$(<"$fixture/log")" == '10|xtask check --parallel' ]]
[[ "$success_output" == 'PASS pre-commit check (CARGO_BUILD_JOBS=10)' ]]
